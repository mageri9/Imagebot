...
def _make_provider(name, generate_mock=None, supports_edits=True):
    return {
        "name": name,
        "provider": type(
            "FakeProvider", (), {"generate": generate_mock or AsyncMock()}
        )(),
...
async def test_success_on_first_provider(patched_deps):
    ok_provider = _make_provider("providerA", AsyncMock(return_value=b"image-bytes"))
...
async def test_fallback_to_second_provider_on_transient_error(patched_deps):
    ...
    ok_provider = _make_provider("providerB", AsyncMock(return_value=b"image-bytes"))
...
async def test_stops_cleanly_when_no_untried_providers_left(patched_deps):
    """Регрессионный тест на фикс шага 3: _get_next_provider возвращает None,
    когда пробовать больше нечего — цикл должен корректно завершиться, а не упасть."""
    failing_provider = _make_provider(
        "providerA", AsyncMock(side_effect=RuntimeError("boom"))
    )
...
async def test_parser_error_blocks_fallback_double_charge_guard(patched_deps):
    """Double-charge guard: ValueError после 'успешного' ответа не должен
    уходить в fallback на другого провайдера."""
    bad_parser_provider = _make_provider(
        "providerA", AsyncMock(side_effect=ValueError("bad response shape"))
    )
    other_provider = _make_provider("providerB", AsyncMock(return_value=b"unused"))
