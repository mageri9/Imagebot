...
**Важно про мульти-фото:** большинство агрегаторов (включая ProxyAPI) принимают только один файл в `images/edits`, даже если документация OpenAI заявляет поддержку нескольких. Поэтому при загрузке нескольких фото бот склеивает их в одно изображение горизонтальной полосой перед отправкой — это работает надёжно на любом агрегаторе. Это поведение единое для обоих провайдеров (`openai_compat` и `genapi`) — оба используют общий метод склейки изображений.

### genapi
При генерации по нескольким фото склеивает их в одну горизонтальную панораму и шлёт одним файлом через multipart/form-data — Gen-API не поддерживает передачу массива файлов в одном запросе. Для мульти-фото с раздельной обработкой каждого снимка используй `openai_compat` (AITunnel).
